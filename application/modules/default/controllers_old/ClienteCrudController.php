...
die("funfo");
    ...
    $select->from($this->_model->getName(),array("cli_id AS id","IF(ISNULL(cli_pf_nome) OR cli_pf_nome='',cli_pj_fantasia,cli_pf_nome) AS value","IF(ISNULL(cli_pf_nome) OR cli_pf_nome='',cli_pj_fantasia,cli_pf_nome) AS label"));
    $select->where("cli_pf_nome  LIKE '%".$_GET['term']."%' OR cli_pj_fantasia LIKE '%".$_GET['term']."%' OR cli_pj_razao LIKE '%".$_GET['term']."%'");
